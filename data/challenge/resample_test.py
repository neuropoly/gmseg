...
def run_main():
    error_files = []
    for original_file in glob.glob("test/*.gz"):
        ...
        nib.save(ni_img, "test/fixed_{}".format(only_fname))
        original_file = "test/fixed_{}".format(only_fname)
           
        print
        print ">>>>>>>> Fixed file -> {}".format(original_file)
            
        cmd = ["sct_resample", "-i {}".format(original_file),
               "-o test_resample/{}".format(only_fname),
